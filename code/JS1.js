...
function slide(slideContainer, targetIndex = curIndex + 1) {
    ...
    //定时切换动画 setInterval() ：按照指定的周期（以毫秒计）来调用函数或计算表达式
    var slideInterval = setInterval(function () {
        if (Math.abs(width - distanceMoved) > Math.abs(step)) { // 边界判定，判断已移动距离以及应移动距离的差与步长关系
            ...
            var directMove = step > 0 ? (curConLeft - width + distanceMoved) : (curConLeft + width -
                distanceMoved); // 正向移动与逆向移动的计算方式不同
            ...
            if (curIndex === imgArr.length) { // index加1，判断是否为最后一张来作边缘处理
                ...
                slideContainer.style.left =
                    `-${slideContainer.offsetWidth - imgArr[imgArr.length - 1].width - imgArr[0].width}px`; // 重置ul
            }
...
(function start() {
    ...
    document.querySelectorAll("#_carousel img").forEach(v => imgArr.push(v)); // 获取所有图片组成数组
